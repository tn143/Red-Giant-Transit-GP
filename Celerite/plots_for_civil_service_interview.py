...
idxt=[(time>(T0+(7*period))-3) & (time<(T0+(7*period))+3)]
...
ax1=plt.subplot(121)
plt.errorbar(time[idxt],lc[idxt],yerr=elc[idxt],fmt='k.',alpha=0.4)#Lightcurve
...
plt.plot(time[idxt],mu,color='C2',alpha=0.6)#Best noise+transit model
plt.fill_between(time[idxt], mu+std, mu-std, color='C2', alpha=0.7, edgecolor="none")
plt.fill_between(time[idxt], mu+(2*std), mu-(2*std), color='C2', alpha=0.4, edgecolor="none")
plt.axhline(y=0,ls='--',color='C0',zorder=-100)
plt.xlim(428.5,431.5)
plt.xlabel('Time (days)',fontsize=18)
plt.ylabel('Flux (ppm)',fontsize=18)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)


ax2=plt.subplot(122,sharey=ax1)
plt.errorbar(time[idxt],lc[idxt]-finalgp[idxt],yerr=elc[idxt],fmt='k.',alpha=0.4)#Lightcurve
plt.plot(time[idxt],final[idxt],'r:',zorder=100,lw=1.5)#Transit model
plt.axhline(y=0,ls='--',color='C0',zorder=-100)
plt.xlim(428.5,431.5)
plt.xlabel('Time (days)',fontsize=18)
...
plt.tight_layout()
plt.tight_layout()
plt.savefig('./CIVIL_SERVICE/GP.png',dpi=200)
plt.show()
