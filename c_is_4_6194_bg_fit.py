...
def initial(numax,f,psd):
	...
	return [a1,a2,b1,b2,P_shot,height,sigma,numax,a3,b3]
...
labels=['a1','a2','b1','b2','W','H','sig','numax','a3','b3']
...
def lnprior(theta,guess):
	snumax=guess[-3]
	a1,a2,b1,b2,W,height,sigma,numax,a3,b3= theta
	if snumax<100:
		if 0.0 <= a1 < 4*guess[0] and 0.0 <= a2 < 4*guess[1] and 0.25*guess[2] <= b1 < 4*guess[2] and 0.25*guess[3] <= b2 < 4*guess[3]  \
		and 0.0 <= W < 1e6 and 0<=height<=1e6 and (snumax/100)<=sigma<(snumax/2) \
		and 0<=numax<3.0*snumax and 0<=a3<=1e9 and 0<=b3<=b1:
			return 0.0
		...
		and 0.0 <= W < 1e6 and 0<=height<=1e6 and (snumax/100)<=sigma<(snumax/2) \
		and 0.5*snumax<=numax<2.0*snumax and 0<=a3<=1e9 and 0<=b3<=b1:
			return 0.0
...
nwalkers, niter, ndim = 100, 1500, int(len(guess))
sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, args=(f,psd,guess))
...
#    a1,a2,b1,b2,W,height,sigma,numax, a3,b3 = 
